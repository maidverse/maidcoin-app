...
class NurseRaid extends skynode_1.DomNode {
    ...
    async load() {
        ...
        const nuresType = StaticDataManager_1.default.getNurseType(raid.nursePart);
        this.content.empty().append(skynode_1.el(".name", nuresType.name), skynode_1.el(".image", { style: { backgroundImage: `url(https://storage.googleapis.com/maidcoin/Nurse/Illust/${nuresType.name}.png)` } }), skynode_1.el(".end-time", `End ${CommonUtil_1.default.displayBlockDuration(raid.endBlock - this.currentBlockNumber)}`), skynode_1.el(".character", skynode_1.el("img", { src: `https://storage.googleapis.com/maidcoin/Nurse/APNG/${nuresType.name}Idle.png`, height: "85" })), skynode_1.el(".duration", skynode_1.el("span.title", "Duration"), skynode_1.el("span", CommonUtil_1.default.displayBlockDuration(raid.duration))), skynode_1.el(".progress"));
        const owner = await Wallet_1.default.loadAddress();
        if (owner !== undefined) {
            const challenger = await NurseRaidContract_1.default.getChallenger(this.raidId, owner);
            this.footer.empty().append(skynode_1.el(".reward", skynode_1.el("h3", "Rewards"), skynode_1.el("img.part", { src: `https://storage.googleapis.com/maidcoin/NursePart/${nuresType.name}.png`, height: "28" }), skynode_1.el(".count", `x 1 ~ ${raid.maxRewardCount}`)), challenger.enterBlock === 0 ? skynode_1.el("a.start-button", ethers_1.utils.formatEther(raid.entranceFee), skynode_1.el("img.icon", { src: "/images/maidcoin.png", height: "20.5" }), "Start", {
                click: () => new SelectMaidPopup_1.default(this.raidId),
            }) : (await NurseRaidContract_1.default.checkDone(this.raidId) === true ? skynode_1.el("a.exit-button", "Exit", {
                click: async () => {
