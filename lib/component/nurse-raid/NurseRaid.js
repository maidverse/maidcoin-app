...
class NurseRaid extends skynode_1.DomNode {
    ...
    async load() {
        const result = await superagent_1.default.post(`https://api.maidcoin.org/nursetypes/${this.raid.nursePart}`);
        const tokenInfo = result.body;
        this.content.empty().append(skynode_1.el(".name", tokenInfo.name), skynode_1.el(".image", { style: { backgroundImage: `url(${tokenInfo.image})` } }), skynode_1.el(".end-time", `End ${CommonUtil_1.default.displayBlockDuration(this.raid.endBlock - this.currentBlockNumber)}`), skynode_1.el(".character", skynode_1.el("img", { src: "/images/test/NoelIdle.png", height: "85" })), skynode_1.el(".duration", skynode_1.el("span.title", "Duration"), skynode_1.el("span", CommonUtil_1.default.displayBlockDuration(this.raid.duration))), skynode_1.el(".progress"));
        const owner = await Wallet_1.default.loadAddress();
        if (owner !== undefined) {
            const challenger = await NurseRaidContract_1.default.getChallenger(this.raidId, owner);
            this.footer.empty().append(skynode_1.el(".reward", skynode_1.el("h3", "Rewards"), skynode_1.el("img.part", { src: `https://storage.googleapis.com/maidcoin/NursePart/${this.raid.nursePart}.png`, height: "28" }), skynode_1.el(".count", `x 1 ~ ${this.raid.maxRewardCount}`)), challenger.enterBlock === 0 ? skynode_1.el("a.start-button", ethers_1.utils.formatEther(this.raid.entranceFee), skynode_1.el("img.icon", { src: "/images/maidcoin.png", height: "21" }), "Start", {
