...
class Nurse extends skynode_1.DomNode {
    ...
    async load() {
        ...
        this.empty().append(skynode_1.el(".slot", skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/Nurse/Face/${nurseType.name}.png` }), skynode_1.el(".name", nurseType.name)), skynode_1.el(".owner", `Owner: ${CommonUtil_1.default.shortenAddress(nuserOwner)}`), skynode_1.el(".lp-amount", "Supported LP : ", this.supportedPower = skynode_1.el("span", CommonUtil_1.default.displayPrice(supportedPower))), this.lifetime = skynode_1.el(".lifetime"), skynode_1.el(".charge-form", skynode_1.el("img.part", { src: `https://storage.googleapis.com/maidcoin/NursePart/${nurseType.name}.png`, height: "50" }), skynode_1.el(".input-container", skynode_1.el("img", { src: "/images/component/charge-multiple-nurse-popup/battery.png", height: "24" }), skynode_1.el("input", {
            change: (event, input) => {
