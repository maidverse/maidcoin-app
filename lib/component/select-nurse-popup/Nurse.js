...
class Nurse extends skynode_1.DomNode {
    ...
    async load() {
        ...
        this.empty().append(skynode_1.el(".slot", skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/Nurse/Face/${tokenInfo.name}.png` }), skynode_1.el(".name", tokenInfo.name)), skynode_1.el(".owner", `Owner: ${CommonUtil_1.default.shortenAddress(this.owner)}`), skynode_1.el(".lp-amount", "Supported LP : ", this.supportedPower = skynode_1.el("span", CommonUtil_1.default.displayPrice(supportedPower))), skynode_1.el("a.support-button", "Support", {
            click: async (event) => {
