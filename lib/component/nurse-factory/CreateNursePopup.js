...
class CreateNursePopup extends skynode_1.Popup {
    ...
    refreshLifetime() {
        if (this.input !== undefined) {
            this.lifetime?.empty().appendText(CommonUtil_1.default.displayBlockDuration(Calculator_1.default.nurseLifetime(this.nurseType, parseInt(this.input.domElement.value, 10), true)));
        }
    }
    async load() {
        const owner = await Wallet_1.default.loadAddress();
        if (owner !== undefined) {
            const nurseTypeInfo = StaticDataManager_1.default.getNurseType(this.nurseType);
            ...
            }), ` / ${balance.toString()}`), skynode_1.el(".range-info", skynode_1.el(".assemble-range", "Assemble", { style: { width: `${100 - rangePercent}%` } }), skynode_1.el(".battery-range", "Battery")), skynode_1.el(".range-container", skynode_1.el(".assemble-range", { style: { width: `${100 - rangePercent}%` } }), this.range = skynode_1.el("input.range", {
