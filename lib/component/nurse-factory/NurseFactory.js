...
class NurseFactory extends skynode_1.DomNode {
    ...
    async load() {
        ...
        }), skynode_1.el("img.part", { src: `https://storage.googleapis.com/maidcoin/NursePart/${nurseType.name}.png`, height: "60" }), balance === undefined ? undefined : skynode_1.el(".part-count", skynode_1.el("span.balance", balance.toString()), ` / ${nurseType.partCount}`), skynode_1.el(".destroy-return", "Destroy Return: ", skynode_1.el("span", CommonUtil_1.default.numberWithCommas(ethers_1.utils.formatEther(nurseType.destroyReturn))), " $MAID"));
        const apr = await Calculator_1.default.nurseAPR(this.nurseType);
