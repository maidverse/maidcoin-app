...
const NurseBatteryPopup_1 = __importDefault(require("./NurseBatteryPopup"));
class NurseFactory extends skynode_1.DomNode {
    ...
    async load() {
        ...
        if (owner !== undefined) {
            const nurseType = await CloneNursesContract_1.default.getNurseType(this.nurseType);
            ...
            this.content.empty().append(skynode_1.el(".image", { style: { backgroundImage: `url(https://storage.googleapis.com/maidcoin/Nurse/Illust/${tokenInfo.name}.png)` } }), skynode_1.el("img.part", { src: `https://storage.googleapis.com/maidcoin/NursePart/${tokenInfo.name}.png`, height: "60" }), skynode_1.el(".part-count", skynode_1.el("span.balance", balance.toString()), ` / ${nurseType.partCount}`));
            this.footer.empty().append(skynode_1.el(".name", tokenInfo.name), skynode_1.el("a.create-button", "Create", {
                click: () => new NurseBatteryPopup_1.default(this.nurseType),
            }));
        }
    }
    delete() {
