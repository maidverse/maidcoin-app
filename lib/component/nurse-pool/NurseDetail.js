...
class NurseDetail extends skynode_1.Popup {
    ...
    async load() {
        ...
        if (owner !== undefined) {
            ...
            this.content.empty().append(skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/Nurse/Illust/${tokenInfo.name}.png` }), skynode_1.el(".name", tokenInfo.name), skynode_1.el(".owner", `Owner: ${CommonUtil_1.default.shortenAddress(nurseOwner)}`), skynode_1.el(".character", skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/Nurse/APNG/${tokenInfo.name}Idle.png`, height: "120" }), skynode_1.el("a.claim-button", skynode_1.el("img.coin-image", { src: "/images/component/nurse-detail/maidcoin.png", height: "29" }), skynode_1.el(".amount", CommonUtil_1.default.numberWithCommas(ethers_1.utils.formatEther(pendingReward))), { click: async () => await CloneNursesContract_1.default.claim([this.nurseId]) })), skynode_1.el(".battery", skynode_1.el("span", "Battery"), skynode_1.el(".range-container", skynode_1.el(".range", skynode_1.el(".bar", {
                style: { width: `${lifetimePercent}%` },
            })), this.lifetime = skynode_1.el(".lifetime")), skynode_1.el("a.charge-button", skynode_1.el("img.image", { src: "/images/component/nurse-detail/charge-button.png", height: "29" }), { click: () => new ChargeNursePopup_1.default(this.nurseId) })), skynode_1.el(".properties", skynode_1.el(".power", skynode_1.el("img", { src: "/images/component/power-icon.png", height: "23" }), skynode_1.el("span", String(nurseType.power))), skynode_1.el(".property.lp-amount", "Total LP Supported: ", skynode_1.el("span", ethers_1.utils.formatEther(supportedPower))), skynode_1.el(".property.lp-amount", "LP Supported By Me: ", skynode_1.el("span", ethers_1.utils.formatEther(supportingAmount)))), skynode_1.el(".controller", skynode_1.el("a.suppport-button", "Support", {
                click: async (event) => {
