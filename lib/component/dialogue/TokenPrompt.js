    ...
    constructor(title, message, confirmTitle, min, max, confirm) {
        super(".token-prompt", title, confirmTitle, () => {
            ...
            if (value.lt(min)) {
                ...
                new Alert_1.default("Error", `Maximum value is ${ethers_1.utils.formatEther(max)}`, "Confirm");
                return false;
            }
            else {
                confirm(value);
        ...
        this.content.append(skynode_1.el("p", message), this.input = skynode_1.el("input.input", { value: ethers_1.utils.formatEther(min) }));
