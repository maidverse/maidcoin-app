...
class TokenPrompt extends Dialogue_1.default {
    constructor(title, message, confirmTitle, min, balance, confirm) {
        super(".token-prompt", title, confirmTitle, () => {
            ...
            if (value.lt(min)) {
                new Alert_1.default("Error", `Minimum value is ${CommonUtil_1.default.displayPrice(min)}`, "Confirm");
                return false;
            }
            else if (value.gt(balance)) {
                new Alert_1.default("Error", `Balance is ${CommonUtil_1.default.displayPrice(balance)}`, "Confirm");
                return false;
            }
            else {
                confirm(value);
        ...
        this.content.append(skynode_1.el("p", message), this.input = skynode_1.el("input.input", { placeholder: `Balance: ${CommonUtil_1.default.displayPrice(balance)}` }), skynode_1.el("a.max-button", "Max", {
