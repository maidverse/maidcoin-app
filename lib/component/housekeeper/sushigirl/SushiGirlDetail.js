...
class SushiGirlDetail extends skynode_1.Popup {
    ...
    async load() {
        ...
        const sushiGirlPower = await NurseRaidContract_1.default.powerOfMaids(SushiGirlsContract_1.default.address, this.id);
        this.content.empty().append(skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/SushiGirl/Character/${this.id}.png` }), skynode_1.el(".name", sushiGirl.name), skynode_1.el(".owner", `Owner: ${CommonUtil_1.default.shortenAddress(sushiGirlOwner)}`), skynode_1.el(".properties", skynode_1.el(".power", skynode_1.el("img", { src: "/images/component/power-icon.png", height: "23" }), skynode_1.el("span", String(sushiGirlPower))), skynode_1.el(".property.origin-power", "Origin Power: ", skynode_1.el("span", String(sushiGirl.originPower))), skynode_1.el(".property.additional-power", "Additional Power: ", this.additionalPower = skynode_1.el("span", String(sushiGirlPower - sushiGirl.originPower))), skynode_1.el(".property.lp-amount", "LP Supported: ", this.supportedLP = skynode_1.el("span", CommonUtil_1.default.displayPrice(supportedLP)))), skynode_1.el(".controller", skynode_1.el("a.power-up-button", "Power Up", {
            click: async (event) => {
        ...
        })), skynode_1.el("a.tweet-button", "Share", { href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`MaidCoin SushiGirl - ${sushiGirl.name}\nhttps://app.maidcoin.org/sushigirls/${this.id}`)}\n\n@maid_coin #GameFi #PlayToEarn #DeFi #YieldFarming #NFT #IEarnMaidCoin`, target: "_blank" }));
    }
    delete() {
