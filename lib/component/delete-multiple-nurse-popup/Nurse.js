...
class Nurse extends skynode_1.DomNode {
    constructor(nurseList, nurseId, owner) {
        ...
        this.changeSupportedPowerHandler = async (id) => {
            if (id.eq(this.nurseId) === true) {
                ...
                this.supportedPowerDisplay?.empty().appendText(ethers_1.utils.formatEther(this.supportedPower));
            }
        };
    ...
    async load() {
        ...
        this.append(skynode_1.el(".slot", skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/Nurse/Face/${nurseType.name}.png` }), skynode_1.el(".name", nurseType.name)), skynode_1.el(".owner", `Owner: ${CommonUtil_1.default.shortenAddress(this.owner)}`), skynode_1.el(".lp-amount", "Supported LP : ", this.supportedPowerDisplay = skynode_1.el("span", ethers_1.utils.formatEther(this.supportedPower))), this.lifetime = skynode_1.el(".lifetime"));
        this.refreshLifetime();
    }
    delete() {
