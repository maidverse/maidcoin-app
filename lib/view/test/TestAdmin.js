...
class Admin {
    constructor() {
        ...
        })), skynode_1.el(".transfer-button-container", skynode_1.el("a.action-button.transfer-button", "Transfer Maid", {
            click: async () => {
                const maidId = prompt("Please enter maid id");
                const to = prompt("Please enter address");
                if (maidId && to) {
                    await MaidsContract_1.default.transfer(to, maidId);
                }
            },
        })), skynode_1.el(".transfer-button-container", skynode_1.el("a.action-button.transfer-button", "Transfer Lingerie Girl", {
            click: async () => {
                const lgId = prompt("Please enter lingerie girl id");
                const to = prompt("Please enter address");
                if (lgId && to) {
                    await LingerieGirlsContract_1.default.transfer(to, lgId);
                }
            },
        }))), skynode_1.el(".admin-form.nurse-type-form", skynode_1.el("h3", "Nurse Type"), skynode_1.el("label", "Part Count", partCountInput = skynode_1.el("input", { placeholder: "Part Count" })), skynode_1.el("label", "Destroy Return", destroyReturnInput = skynode_1.el("input", { placeholder: "Destroy Return" })), skynode_1.el("label", "Power", powerInput = skynode_1.el("input", { placeholder: "Power" })), skynode_1.el("label", "Lifetime", lifetimeInput = skynode_1.el("input", { placeholder: "Lifetime" })), skynode_1.el(".action-button-container", skynode_1.el("a.action-button.create-nurse-type-button", "Create Nurse Type", {
            click: async () => {
